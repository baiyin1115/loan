package com.zsy.loan.admin.core.qr;
...
public class ImgQrTool {
  ...
  public static void encode(String content, int width, int height, String srcImagePath,
    ...
    try {
      ImageIO
          .write(genBarcode(content, width, height, srcImagePath), "jpg", new File(destImagePath));
    } catch (IOException e) {
      e.printStackTrace();
  ...
  public static void createSimpleQr(String content, int width, int height, String destImagePath) {
    ...
    try {
      ...
      BitMatrix bitMatrix = new MultiFormatWriter()
          .encode(content, BarcodeFormat.QR_CODE, width, height, hints);// 生成矩阵
    ...
    try {
      ...
      if (para.getWordContent().length() > qrImageWidth / para.getWordSize()) {//每行限制的文字个数
        ...
        fontsImageGraphics.drawString(
            para.getWordContent(),
            ((qrImageWidth / para.getWordSize() - para.getWordContent().length()) / 2) * para
                .getWordSize() + 20, //总长度减去字长度除以2为向右偏移长度
            para.getWordSize());
      }

      // 从图片中读取RGB
      int[] ImageArrayFonts = new int[qrImageWidth * fontsImageHeight];
      ImageArrayFonts = imageWithFonts
          .getRGB(0, 0, qrImageWidth, fontsImageHeight, ImageArrayFonts, 0, qrImageWidth);

      int[] ImageArrayQr = new int[qrImageWidth * qrImageHeight];
      ImageArrayQr = qrImage
          .getRGB(0, 0, qrImageWidth, qrImageHeight, ImageArrayQr, 0, qrImageWidth);

      // 生成新图片
      BufferedImage ImageNew = new BufferedImage(qrImageWidth, qrImageHeight + fontsImageHeight,
          BufferedImage.TYPE_INT_RGB);
      ImageNew.setRGB(0, 0, qrImageWidth, fontsImageHeight, ImageArrayFonts, 0,
          qrImageWidth);// 设置上半部分的RGB
      ImageNew.setRGB(0, fontsImageHeight, qrImageWidth, qrImageHeight, ImageArrayQr, 0,
          qrImageWidth);// 设置下半部分的RGB
